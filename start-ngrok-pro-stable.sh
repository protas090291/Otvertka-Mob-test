...
# Стабильный запуск Expo с Ngrok Pro (платный, самый надежный)
# Требуется: Ngrok Pro аккаунт и authtoken

cd "$(dirname "$0")"

echo "🚀 Запуск Expo с Ngrok Pro (стабильный tunnel)"
echo ""

# Проверка ngrok (проверяем в нескольких местах)
export PATH="$HOME/.local/bin:$PATH"
if ! command -v ngrok &> /dev/null; then
    echo "❌ ngrok не установлен"
    echo "Установите: brew install ngrok/ngrok/ngrok"
    echo "Или скачайте с https://dashboard.ngrok.com/get-started/setup/macos"
    exit 1
fi
echo "✅ ngrok найден: $(which ngrok)"
...
if ! ngrok config check &> /dev/null; then
    ...
    echo "2. Выберите план (Pro от \$8/месяц)"
...
# Запускаем Ngrok Pro tunnel с автоматическим перезапуском
echo "🌐 Запуск Ngrok Pro tunnel..."
screen -dmS expo-ngrok bash -c "
    cd '$(pwd)'
    while true; do
        echo '🌐 Запуск Ngrok Pro tunnel...' >> ngrok.log
        ngrok http 8081 --log=stdout 2>&1 | tee -a ngrok.log
...
for i in {1..60}; do
    ...
    if [ ! -z "$NGROK_URL" ]; then
        ...
        echo ""
        echo "✅ Готово! Ngrok Pro tunnel работает стабильно"
        echo "📱 URL: $EXP_URL"
        echo "💡 Этот tunnel работает без обрывов благодаря Ngrok Pro"
        echo ""
        echo "📋 Управление:"
